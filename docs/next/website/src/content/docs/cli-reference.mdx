...
`herdr status` also prints a `sessions:` table covering every session on this machine — running and stopped — with each session's protocol, compatibility, and version, and a `*` marking the session the `server:` block above describes.
