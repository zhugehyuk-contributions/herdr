...
`herdr status` 还会打印一个 `sessions:` 表格,列出本机上的每个会话(运行中和已停止的都包括),显示各会话的协议、兼容性和版本,`*` 标记上面 `server:` 块所指的那个会话。
