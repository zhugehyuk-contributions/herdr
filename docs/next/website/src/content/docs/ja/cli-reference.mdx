...
`herdr status` は、このマシン上のすべてのセッション(実行中・停止中の両方)を `sessions:` テーブルとして併せて表示します。各セッションのプロトコル、互換性、バージョンが並び、`*` は上の `server:` ブロックが指しているセッションを示します。
