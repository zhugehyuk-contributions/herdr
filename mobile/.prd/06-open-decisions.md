...
## 결정 8·9·10 — "유저 판정 3건"을 확정한다 (2026-08-25)

셋 다 유저 게이트로 올려뒀지만, 판정 근거가 이미 이 레포 안에 있었다. 게이트를 잘못 세운 것이다.

### 결정 8 — 액세서리 키는 **19개 유지** (목업 6개와 다른 채로)

목표 조건 2가 이 분기를 직접 결정한다: *"orca mobile 와꾸를 이용하여 **대부분 모르겠으면 orca
mobile에서 결정한 내용을 사용**"*. 그리고 `01-spec.md:55`는 키 액세서리 행을 **오너 미검증**으로
표기한다 — 정확히 "모르겠으면"에 해당한다. 구현의 19개는 임의 확장이 아니라
`terminal-accessory-herdr-keys.ts:184`가 말하듯 **orca의 행에서 herdr가 표현 못 하는 것을 뺀 것**이다.
→ 조건 2의 적용 결과가 곧 현 구현이다. 목업이 6개인 것은 **차이로 남기되 결함이 아니다.**
...
3택은 ①observe(현재) ②attach ③서버측 클라이언트별 가상 크기였다.
- ②는 **유저의 실제 세션 그리드를 폰이 바꾼다** — 데스크톱에서 보던 화면이 폰 때문에 리사이즈된다.
  §2.3의 observe-only 결정과 정면 충돌하고, 그 결정이 B7(stale attach 회수)을
  애초에 발생하지 않게 하고 있다(`03-blockers.md:9`).
- ③은 API에 없다. 서버 작업이며, 있어도 ②의 변형이다.
- ①은 이미 `284c0558`이 수리한 형태다 — 핀치가 `term.resize`를 부르지 않고 지속 배율로만 확대한다.

→ **폰은 남의 세션을 리사이즈하지 않는다**가 이 앱의 계약이다. 뒤집으려면 M6급 제어 도입이 선행한다.
