...
| # | 채택 | 근거 | 재검토 트리거 |
|---|---|---|---|
| **1 렌더러** | **`TerminalAnsi` + xterm WebView** | 코덱 부담 최소(손코덱 3종) · orca 재사용 최대(전제 2) · 동결 대상 5필드 · **2026-08-22 리시트로 경로 자체가 실증됨** | **B13 대역폭**(18.6 B/cell, 무압축)이 셀룰러에서 실사용 불가로 판명되면. 완화 ①(ANSI 분기에도 deflate)을 먼저 시도하고, 그래도 안 되면 `SemanticFrame`으로 전환 — 그때 코덱 포팅 비용이 되살아난다 |
