...
| 목업 화면 | 소견 |
|---|---|
| ②nodes · ④노드상세 · ⑤워크스페이스 · ⑥터미널 · ⑦agents | 있음 |
| ③add remote 시트 | **`settings` 안에** — 목업은 독립 시트 |
| ①QR 스캔 → add remote 프리필 | **없음** (코드에 `QR` 참조 0건). 데스크톱 herdr 클라이언트의 QR 발급도 필요 |

요소 차이 3건(확인 대상): 노드 행이 `2 working · 1 blocked` 분해 대신 `3 unknown` /
pane 행에 목업이 명시한 `└ 요약 줄 = pane.read`가 안 보임 / 터미널 헤더의 **`History` 버튼은
목업에 없다**(M6c에서 구현자가 추가 = `초과`).
