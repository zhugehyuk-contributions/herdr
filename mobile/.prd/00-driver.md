...
참조에게 빼앗기던 것이 원인이었고 `scripts/pods-install.sh`가 그 충돌만 수리한다.
**iOS QA 11차가 PASS·MUST-FIX none으로 판정**했고, 판정관이 bare `pod install`로 되돌려
동일 서명을 재현한 뒤 스크립트로 복구까지 확인했다 — 우연 통과가 아니다.

**조건 3은 이제 양 플랫폼에서 성립한다**: Android 7차 PASS · iOS 11차 PASS, 둘 다 별도 에이전트
판정이고 iOS에 더 이상 "마이너스 `expo-camera`" 라벨이 없다.
남은 것은 실기기 게이트뿐이다 — 시뮬레이터엔 카메라가 없어 **실제 QR 스캔은 iOS에서 판정불가**이고
(Android에서는 실측 완료), 권한 다이얼로그 버튼 조작도 자동화 수단이 없다.

**유저 게이트**(진짜 남은 것만): iOS 코드사이닝 · iOS 실기기 · M5 Expo 계정.
자격증명·하드웨어·계정이라 내가 대신할 수 없는 것들이다.

구 "유저 판정 3건"(터미널 사이징 · pane 요약줄 소스 · 액세서리 키 19 vs 6)은
**게이트를 잘못 세운 것이었다** — 판정 근거가 이미 레포 안에 있었다. `06-open-decisions.md`
결정 8·9·10으로 확정했고, 전부 현 구현 유지다. iOS 빌드 수리도 완료(§조건 5).
