...
| M6a 스와이프 리타겟 | ✅ + 근인 수리 `9a256d0a` | **PASS** §FF — 스와이프분 184ms(<200ms), 재연결 0, 양끝 no-op | **FAIL** §HH — 제스처는 RN까지 도달하는데(대조실험 −425.7pt) pan이 발화 안 함. 계측+후보수리 대기 |
| M6b 제어 승격 | ⛔ **삭제** — §2.3과 모순, M3가 그 결정 위에 출하됨 | — | — |
| M6c 스크롤백 | ✅ `cde4eb2b` | **PASS** §GG — 버튼 y 53→169, Refresh 재읽기·B8 캐시 양방향 확증 | 미착수 |
