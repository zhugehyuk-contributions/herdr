...
| M2 읽기 전용 앱 | ✅ | **✅ 5/5** | **FAIL** — a·d PASS, **b·e FAIL(§U)**, c 판정 불가 |
| **M2b 원격 설정 + 키 보관** | ✅ | **✅ 3/3** | 판정 불가 — **서명 게이트** |
| M3 입력 | ✅ | **✅ PASS 4/4** | ❌ (M2 (b)에 막힘) |
| M4 transport 생존성 | ✅ + exec 데드라인 (`b411ef04`) | **QA 진행 중** | ❌ |
...
1. **§U 수리 — iOS 터미널이 프레임을 받고도 안 그린다.** M2 (b)·(e)를 막고 있고, 따라서 **목표 3의
   iOS 절반 전체**를 막고 있다. 근거·좁혀둔 것 = `.prd/09` §U. 1순위 가설은 WebGL이 "붙었는데 안 그린다"
   (`terminal-webview-webgl-recovery-injected.ts:18-56`에 그 폴백이 없다).
   → 수리 후 **iOS M2 (b)(c)(e) 재QA**(별도 에이전트).
2. **M4 Android 판정 수령** — 진행 중. 기내모드 10사이클 완료, 30분 백그라운드 관측 중.
   판정이 오면 표 갱신. iOS M4는 **시뮬레이터로 닫히지 않는다**(기내모드·셀룰러 재현 불가) — 실기기 필요.
