...
| M2 읽기 전용 앱 | ✅ | **✅ 5/5** | **FAIL** — a·d PASS · **b FAIL(§X)** · c·e 판정 불가 |
| **M2b 원격 설정 + 키 보관** | ✅ | **✅ 3/3** | 판정 불가 — **서명 게이트** |
| M3 입력 | ✅ | **✅ PASS 4/4** | ❌ (M2 (b)에 막힘) |
| M4 transport 생존성 | ✅ + 실패-원격 재다이얼 (`8f794748`) | **FAIL** — 3항목 PASS, 강제종료 1건(§W) → 수리됨, **재QA 필요** | ❌ (시뮬로 안 닫힘) |
...
1. **iOS `init()`이 완주하지 않는다 — 이것 하나가 목표 3의 iOS 절반 전체를 막고 있다.**
