...
| M6a 스와이프 리타겟 | ✅ + 근인 수리 `9a256d0a` | **PASS** §FF — 스와이프분 184ms(<200ms), 재연결 0, 양끝 no-op | **제품 결함 확정** §KK — 같은 WKWebView 대조군(페이지 미청취 시 dx=−229 전환 성공 / 청취 시 dx=0)으로 하네스 가설 사망. 내 양보 가드가 스냅 때문에 죽은 코드였음 → `userScale > 1`로 수리, 재QA 대기 |
