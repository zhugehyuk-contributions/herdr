...
**iOS의 유일한 미결은 조건 5**이고, **SPM 이음매를 안 건드리는 우회로가 있다** —
QR 내용을 JSON → `herdr://pair?…` URL로 바꾸면 **시스템 카메라**가 읽어 앱을 연다(`.prd/12` §⭐).
폰 쪽은 딥링크 라우트 하나이고 파서·인계·폼 프리필은 그대로 재사용된다. 그게 아니면 아래. `expo-camera` 팟이 `Pods.xcodeproj`를 못 열게 만들고
