...
**iOS 조건 5는 해소됐다** (2026-08-24). `expo-camera`가 `Pods.xcodeproj`의 프로젝트 객체를
SPM 참조에게 빼앗기던 것이 원인이었고, `scripts/pods-install.sh`가 그 충돌만 수리한다.
카메라를 켠 채로 `BUILD SUCCEEDED`. 딥링크 우회로는 더 이상 필요 없어 대안으로만 남긴다.
