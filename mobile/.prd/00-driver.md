...
| M6a 스와이프 리타겟 | ✅ + 근인 수리 `9a256d0a` | **PASS** §FF — 스와이프분 184ms(<200ms), 재연결 0, 양끝 no-op | **QA 진행 중** |
| M6b 제어 승격 | ⛔ **삭제** — §2.3과 모순, M3가 그 결정 위에 출하됨 | — | — |
| M6c 스크롤백 | ✅ `264b4479` + safe-area 수리 | **재QA 대기** (2차 FAIL §FF — 닫기 버튼이 상태바 아래, §D1 재발. 데이터·렌더·스크롤은 전부 통과) | 미착수 |
