...
**남은 실제 작업**:
- **M6a** 스와이프 → `retarget(paneId)`. 1차 FAIL → **근인 수리** `9a256d0a` (`nestedScrollEnabled`가 터치다운에서 취소, §DD). **재QA 대기.** 지연 592ms는 별건 — WebView 문서 재생성이 지배항
- **M6c** 스크롤백 열람. 1차 FAIL → **구현** `264b4479` — `pane.read{recent_unwrapped, 1000}` + 터미널을 덮는 별도 오버레이 (§DD). **재QA 대기.** 최대 위험 = 서버 버퍼가 비어 있으면 다른 이유로 또 FAIL
...
> ⛔ **2026-08-23 QA가 이 절을 반증했다 (`.prd/09` §CC).** 배관은 있지만 **스크롤할 것이 생기지 않는다** —
> `src/protocol/render_ansi.rs:613`이 셀마다 **절대 커서 위치**(`ESC[{row};{col}H`)를 쓰므로 터미널이
> 스크롤하지 않고 제자리에서 덮어써진다. 실측 `scrollHeight == clientHeight`(스크롤백 0행).
> **`scrollback: 5000`은 무동작이다.** 아래 "구현돼 있는 것" 목록은 사실이지만 **결론이 틀렸다.**
