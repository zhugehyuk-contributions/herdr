...
use crate::protocol::{
    ...
    CursorState, FrameData, RenderEncoding, ServerMessage, TerminalSessionMode, PROTOCOL_VERSION,
};
...
/// The message that moves an established session (M6/B6). Two vectors, because the interesting
/// byte is the *nested* mode tag and the two variants differ in length: `Observe` is a bare tag and
/// `Control` carries a `bool` behind it. A hand-written encoder that treats the mode as one byte
/// works for observe and truncates control, and truncation is the failure that decodes as garbage
/// rather than as an error.
fn retarget_terminal_vectors() -> Vec<Value> {
    let observe = ClientMessage::RetargetTerminal {
        target: FIXTURE_OBSERVE_TARGET.to_owned(),
        mode: TerminalSessionMode::Observe,
    };
    let control = ClientMessage::RetargetTerminal {
        target: FIXTURE_OBSERVE_TARGET.to_owned(),
    ...
    vec![
        json!({
            "name": "retarget_terminal_observe",
            "direction": "client_to_server",
            "message": "RetargetTerminal",
            "field_order": ["target", "mode"],
            "fields": {
                "target": FIXTURE_OBSERVE_TARGET,
                "mode": "Observe",
                "mode_tag": variant_tag(&TerminalSessionMode::Observe)
            },
            "framed_hex": hex(&framed(&observe)),
            "note": "Appended at the ClientMessage TAIL (tag 14): every tag 0..13 kept its value, \
                     which is this fork's merge contract \
                     (client_message_wire_tags_preserve_protocol_15_order). \
                     `TerminalSessionMode::Observe` is a UNIT variant — one tag byte, nothing after \
                     it. Encoding it as tag+false appends a byte the server reads as the start of \
                     the next message. Unlike ObserveTerminal, this is legal only on a connection \
                     that ALREADY has a terminal session; on a fresh one the server refuses it with \
                     a Notify and keeps the connection (src/server/headless.rs, \
                     retarget_terminal_client)."
        }),
        json!({
            "name": "retarget_terminal_control",
            "direction": "client_to_server",
            "message": "RetargetTerminal",
            "field_order": ["target", "mode"],
            "fields": {
                "target": FIXTURE_OBSERVE_TARGET,
                "mode": "Control",
                "mode_tag": variant_tag(&TerminalSessionMode::Control { takeover: true }),
                "takeover": true
            },
            "framed_hex": hex(&framed(&control)),
            "note": "`Control { takeover }` is a tag byte followed by the bool, so this frame is \
                     exactly one byte longer than retarget_terminal_observe. ⚠️ Control RESIZES the \
                     shared PTY to this client's Hello grid and takes its lock \
                     (attach_terminal_client) — a phone that declares its own screen size here \
                     reshapes a desktop pane. The mobile contract is to hold it across one input \
                     and retarget straight back to Observe, which restores the grid recorded at \
                     promotion (release_terminal_control)."
        }),
    ]
}
...
fn fixtures_decode_back_through_the_rust_codec() {
    ...
    // M6/B6. The one thing a hand-written encoder gets wrong here is the NESTED mode tag, so the
    // check that matters is the length delta: `Control` is exactly one byte longer than `Observe`,
    // and both decode with nothing left over.
    let mut lengths = Vec::new();
    for (name, expected_mode) in [
        ("retarget_terminal_observe", TerminalSessionMode::Observe),
        (
            "retarget_terminal_control",
            TerminalSessionMode::Control { takeover: true },
        ),
    ] {
        let vector = vectors
            .iter()
            .find(|vector| vector["name"] == name)
            .unwrap_or_else(|| panic!("{name} vector"));
        let bytes = decode_hex(vector["framed_hex"].as_str().expect("framed_hex"));
        let (declared, payload) = split_frame(&bytes);
        assert_eq!(declared, payload.len(), "{name} length prefix disagrees");
        let (message, consumed): (ClientMessage, usize) =
            bincode::serde::decode_from_slice(payload, bincode::config::standard())
                .unwrap_or_else(|err| panic!("{name} decodes as ClientMessage: {err}"));
        assert_eq!(consumed, payload.len(), "{name} has trailing bytes");
        assert_eq!(
            message,
    ...
    assert_eq!(
        lengths[1],
        lengths[0] + 1,
        "Control carries a bool behind its tag and Observe does not; a codec that encoded one \
         byte for both would make these equal"
    );
    assert_eq!(
        document["enums"]["ClientMessage"]["RetargetTerminal"].as_u64(),
        Some(14),
        "RetargetTerminal must stay at the tail; 0..13 are frozen by wire.rs"
    );
}
